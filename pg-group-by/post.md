...
Я выбрал 2, т.к. подход хэширования более оптимальный с точки зрения лишнего IO. Проблема здесь в том, что этим самым мы заставляем группировку работать с типами сравниваемыми И хэшируемыми, но я считаю, что большая часть типов такими и является.

> В общем-то вся реализация - это битва с компромиссами. В финальном варианте, я накладываю ограничения на сами типы - они должны быть И сортируемыми И хэшируемыми. Альтернативный вариант - накладывать ограничения на агрегаты (их состояние) - они должны быть сериализуемыми.
...
1. При переполнении сбрасываем *кортеж* в определенную партицию, чтобы после читаем каждую партицию и создаем для нее структуру
   - Ограничение на типы: должны быть хэшируемыми и сравниваемыми
2. При переполнении сбрасываем *индекс* (с сериализованными состояниями агрегатов), создавая отсортированную последовательность, а в конце выполяем слияние с вызовом функции объединения (combine)
   - Ограничение на агрегаты: должны быть сериализуеыми и объединяемыми
...
...мы не сможем выполнить сортировку в том же узле, т.к. 3-ий GS содержит атрибуты из 2 других, поэтому в таких случаях придется выполнять явную сортировку всех GS, т.е. нужен явный узел сортировки. Поэтому я принял решение, что новая стратегия будет поддерживать только 1 GS.
...
Единственное, что можно сказать - реализация этого индекса вынесена в отдельный файл и не зависит от модуля группировки (так же сделано и для хэш-таблицы кортежей, которая используется для группировки хэшированием). Просто приведу саму структуру.
...
explain select num from numerictbl order by num;
                                QUERY PLAN                                 
---------------------------------------------------------------------------
 Sort  (cost=132154.34..134654.34 rows=1000000 width=12)
   Sort Key: num
   ->  Seq Scan on numerictbl  (cost=0.00..15406.00 rows=1000000 width=12)
(3 rows)
```
...
Как видно, у меня все выполнение заняло чуть меньше секунды.
...
Индекс реализован. Теперь приступим к основной части - коду группировки. Вообще, ее логика слишком сильно похожа на хэширование, поэтому я не стал заморачиваться и просто (почти построчно) скопировал эту реализацию:
...
- код сброса - переиспользуются и адаптируются функции хэширования (`agg_spill_tuple`)
...
То есть нам необходимо выполнить (если это можно так назвать) инверсию управления - теперь мы сами определяем что и когда будет сбрасываться на диск. Для этого используется 2 функции, которые обрамляют код записи кортежей (уже отсортированных):
