...
По факту, основное требование - обнаружение изменения даже одного байта сообщения. 
...
2. Если разряд делимого 1:
   1. Вычитаем делитель
3. Переходим к следующему разряду
4. Если длина оставшейся части меньше делителя:
   1. Оставшееся число - остаток
   2. Конец
5. Переходим к первому этапу
...
```cs
uint InitialPerByteRegister = 0xFFFFFFFF;
