...
### TODO: написать про их использование
...
SSD хранит незаписанные данные в DRAM кэше (т.е. оперативке своей)

ECC, Error Correction Codes - коды коррекции ошибок.

Сказать про обманывающий (fsync) контроллер.

Есть 2 паттерна доступа: последовательный и случайный (нужно ли?)

## RAID

Тут тоже что-нибудь поискать


## HDD

Единица чтения - сектор (512-4096 байт)

## SSD

Есть Block - 2-размерный массив из ячеек памяти. Содержит 64-128 страниц

Есть Page - ряд транзисторов в блоке (Block) (почти как сектор в HDD) - единица чтения.

Запись медленнее, но контроллер может кэшировать запись в RAM.

Контроллер берет на себя обязанность делать так, чтобы запись происходила равномерно (т.е. не один и тот же блок перезаписывался).


Read, Program - работают со страницами
Erase - сразу со всем блоком (все 0 выставляем)




Мин. единица чтения - Page
Мин. единица записи - Block
