...
Справа идет диаграмма зависимостей - то, как приложение должно себя вести в чистой архитектуре. Она представлена в формате UML и стрелки представляют соответственно связи между объектами.
...
Наша главная задача - бронировать и покупать места для посетителей на сеансах. По идее, сеанс содержит в себе интервал показа фильма, сам фильм, зал и другие данные. (!) Но наша обязанность - это бронирование мест, поэтому для нас сеанс - это просто набор мест с их состоянием. Реализовать сеанс можно следующим образом:
...
- В коде просто так состояние не изменить — все выставляемые наружу свойства неизменяемые (get-only, IReadOnlyCollection);
...
Я сторонник богатой модели, так как она позволяет выразить логику в коде яснее (данные и алгоритмы хранятся вместе) и может обеспечить сохранность состояния ([Always Valid Model](https://codeopinion.com/always-valid-domain-model/)).
...
Но существуют и другие подходы (package by layer, package by ). Обзор на них можно посмотреть в этой [статье](https://habr.com/ru/articles/683456/).
