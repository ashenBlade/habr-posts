...
Перед началом работы выставляются флаги событий, которые мы отслеживаем, `40` - установить маску для ожидаемых событий. Отслеживается 4 события: нажатие на клавиши (клавиатуры), нажатие на кнопку (мыши), событие отладки и перерисовка окна (да, оконная подсистема внутри ядра).
...
Когда мы получаем исключение, то проверяем, что код исключения - `0x0D` (#GP, General Protection Fault), его генерирует `int3`. А затем проверяем, что под нами находится либо:
