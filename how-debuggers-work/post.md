...
TODO: код ad-hoc отладчика на C
...
| Name                              | Info              | Size/Type                                         |
| --------------------------------- | ----------------- | ------------------------------------------------- |
| Смещение в `strtab` названия типа | Информация о типе | Размер или ID типа (в зависимости от поля `Info`) |

Все поля имеют размер в 4 байта. `Size/Type` - это union. `Name` понятно что означает, но главная часть - `Info`. Это 4-байта поделенные на 3 поля:
...
| Name     | Offset                                 | Type    |
| -------- | -------------------------------------- | ------- |
| Имя поля | Смещение относительно начала структуры | ID типа |
